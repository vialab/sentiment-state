...
function populate(username){
    ...
    var link = "http://vialab.science.uoit.ca:800/twitter/"+username;
    ...
    d3.json(link, function(error, data) {
        ...
        var join = parseDate(data[0].user.created_at);
        ...
        d3.select("#info").append("text").style("opacity", 0.0)
            .html( "<br>Joined Twitter: "+ joined(join)+"</br>Location: "+ data[0].user.location+
        ...
        var circlegroup = focus2.append("g");
        ...
        circlegroup.selectAll(".dot")
            ...
            .attr("r", function(d) { if(check(d.text) == true){ return 3.5;}else{return 3.5;} })
            .attr("cx", function(d) { return x(d.date); })
            .attr("cy", function(d) { return y(d.totalScore);})
            .style("opacity", function(d) { return dotVis(d.totalScore);})
            ...
            .on("mouseover", function(d) {
                div.transition()
                    .duration(500)
                    .style("opacity",0.9);
                d3.select(this).attr('r', 6).transition().duration(600).style("opacity",1.0);
        ...
        var pieChart = pieD.datum(values).selectAll("path")
            ...
            .on("mouseout", function(d) {
                div.transition()
                    .duration(500)
                    .style("opacity", 0);
                d3.select(this).transition().duration(400).style("opacity",0.6);
            })
            .on("click", function(d){
    ...
    function brushed() {
        ...
        var circlegroup = focus2.selectAll("circle")
            .attr("cx", function(d) { return x(d.date); })
            .attr("cy", function(d) { return y(d.totalScore);});
    }
    ...
    function check(string){
        var url = /[A-Za-z]+:\/\/[A-Za-z0-9-_]+\.[A-Za-z0-9-_:%&~\?\/.=]+/g;
        var twitterUser = /[@]+[A-Za-z0-9-_]+/g;
        var hashtag = /[#]+[A-Za-z0-9-_]+/g;

        if(url.test(string)== true || hashtag.test(string)== true || twitterUser.test(string)== true)
            return true;
    }
    ...
    function dotVis(scoretemp){
        return 0.55; // makes all the opcaity the same
        if(scoretemp>0 && scoretemp<=2)
            return 0.3;
